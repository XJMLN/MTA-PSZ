...
local NATIVE_RESOLUTION		=	{ 800, 600 } -- put your screen resolution here to fit the gridlists to all resolutions (ex: { 1366, 768 } )
        ...
        if #dxGrid.items > 0 then
            ...
            for index, data in ipairs ( dxGrid.items ) do
                ...
                if data.vis then
                    ...
                    dxDrawRectangle ( data.x, data.y, data.w, data.h, tocolor ( 12, 12, 12, 136 ), data.pg );

                    -- Draw the column bar
                    dxDrawRectangle ( data.x, data.y, data.w, 30 % data.h, tocolor ( 12, 12, 12, 216 ), data.pg );

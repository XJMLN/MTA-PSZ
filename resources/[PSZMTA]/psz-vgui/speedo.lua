...
Vgui - licznik w pojezdzie

@author Jakub 'XJMLN' Starzak <jack@pszmta.pl
@package PSZMTA.psz-vgui
...
function showSpeedometer()
    ...
    if (vm~=481 and vm~=509 and vm~=510) then 
...
addEventHandler("onClientVehicleEnter", g_root,
	function(thePlayer)
      --  if (getElementData(thePlayer,"justConnected")) then return end
...
addEventHandler("onClientResourceStart", g_rootElement,
	function ()
        if (getElementData(g_Player,"justConnected")) then return end
        initGui()
        guiSetVisible(disc, false)
        setTimer(function()
            local w, h = guiGetScreenSize()

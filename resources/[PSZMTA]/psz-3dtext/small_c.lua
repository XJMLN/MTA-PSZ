...
function refreshNearbyctexts()
    teksty=getElementsByType ( "ctext", getRootElement(), true )
...
setTimer(refreshNearbyctexts, 2000, 0)
            ...
            if (t and isElement(t) and getElementDimension(t)==pd and getElementInterior(t)==pi) then
                ...
                if dist < defdd then
                    ...
                    if scx and scy and isLineOfSightClear ( px, py, pz, cx, cy, cz, true,false, true, true ) then
                        ...
                        local ctext  = getElementData ( t, "ctext" )
                        if not ctext or ctext == "" then return end
                        local c     = getElementData ( t, "rgba" )
                        if c and type ( c ) == "table" then r = c[1] g = c[2] b = c[3] alpha = c[4] end
                        local ac=(r+g+b)/3
                        dxDrawText ( ctext, scx+2, scy+2, scx, scy, tocolor ( 255-ac, 255-ac, 255-ac, alpha ), scale, "default-small", "center", "center" )
                        dxDrawText ( ctext, scx-2, scy+2, scx, scy, tocolor ( 255-ac, 255-ac, 255-ac, alpha ), scale, "default-small", "center", "center" )
                        dxDrawText ( ctext, scx+2, scy-2, scx, scy, tocolor ( 255-ac, 255-ac, 255-ac, alpha ), scale, "default-small", "center", "center" )
                        dxDrawText ( ctext, scx-2, scy-2, scx, scy, tocolor ( 255-ac, 255-ac, 255-ac, alpha ), scale, "default-small", "center", "center" )
                        dxDrawText ( ctext, scx, scy, scx, scy, tocolor ( r, g, b, alpha ), scale, "default-small", "center", "center" )
                    end
                end
            end
        end

...
local players = getElementsByType( "player" )
			for key, player in ipairs( players ) do
				if not getPlayerTeam( player ) or not (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) or serverInfo.forcehideteams then
					...
					for key, column in ipairs( scoreboardColumns ) do
                        ...
                        elseif column.name == "character" then
                            content = getElementData(player,column.name)
                            if (not content or not content.gg_nazwa or not content.gg_tag) then content = "" end
                            content = content.gg_tag
                        elseif column.name == "reputacja" then 
                        	content = (getElementData(player,"bad_reputation") or 0) + (getElementData(player,"good_reputation") or 0)
		...
		if scoreboardDrawn then
			...
			local y = topY+s(5)
			dxDrawText( "http://pszmta.pl - forum serwera", topX+s(5), y+s(1), topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cServerInfo, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "left", "top", false, false, drawOverGUI )
			...
			while ( index < firstVisibleIndex+maxPerWindow and scoreboardContent[index] ) do
				...
				elseif element and isElement( element ) and getElementType( element ) == "player" then
					...
					for key, column in ipairs( scoreboardColumns ) do
                        ...
                        if content and column.name == "id" then
                            if getElementData(element,"vip") then
                                r,g,b = 255,215,0
                            end
                        end
						if content then						

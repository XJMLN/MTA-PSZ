	...
	if isPlayerMapVisible() then
		...
		for k,v in ipairs(blips) do
			...
			if isElement(attached) and getElementType(attached)=="player" then
				if attached ~= localPlayer and getElementDimension(localPlayer)==getElementDimension(attached) and getElementInterior(localPlayer)==getElementInterior(attached) and getElementAlpha(attached)>0 then 

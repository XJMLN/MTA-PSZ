...
function eventTriggered ( source,killer,weapon,bodypart,suicide,usedVehicle )
	...
	if ( killer ) then
		if suicide then
			...
			if weaponName then
				outputConsoleKillMessage ( "* "..playername.." killed himself. ("..weaponName..")" )
			else
				outputConsoleKillMessage ( "* "..playername.." killed himself."..extra )
			end
		else
			local weaponName = getWeaponNameFromID ( weapon )
            local Kplayername = string.gsub(getPlayerName(killer),"#%x%x%x%x%x%x","")
			if weaponName then
                outputConsoleKillMessage ( "* "..Kplayername.." killed "..playername..". ("..weaponName..")" )
			else
                outputConsoleKillMessage ( "* "..Kplayername.." killed "..playername.."."..extra )
			end
		end
	else
        outputConsoleKillMessage ( "* "..playername.." died."..extra )
	end
	--
end

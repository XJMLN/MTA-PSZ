...
function createRadioListGui ()
	if usingGui == false then
		...
		radioListGui.window = guiCreateWindow(radioListGui.windowX,radioListGui.windowY,radioListGui.windowsizeX,radioListGui.windowsizeY,"Radio",false)
		...
		radioListGui.usedColumn = guiGridListAddColumn(radioListGui.usedGrid,"Używane stacje",0.87)
		...
		radioListGui.notUsedColumn = guiGridListAddColumn(radioListGui.notUsedGrid,"Nie używane stacje",0.87)
...
addCommandHandler ("stacja", createRadioListGui)
addCommandHandler ("radia", createRadioListGui)
	...
	if button == "left" and state == "up" then
		...
		if source == radioListGui.notUsedGrid then
			...
			if selectedRow ~= -1 then
				...
				--[[	
					if stationsNames[stID] == "Youtube Player" then 
						table.insert(usedStations,1, stID)
						guiGridListClear(radioListGui.usedGrid)
						for k, v in ipairs(usedStations) do
							local row = guiGridListAddRow(radioListGui.usedGrid)
							guiGridListSetItemText(radioList.usedGrid, row, radioListGui.usedColumn, stationsNames[v], false, false)
						end
					else
						local row = guiGridListAddRow(radioListGui.usedGrid)
						guiGridListSetItemText( radioListGui.usedGrid, row, radioListGui.usedColumn, stName, false, false)

...
local function isPlayerVIP(checker)
	local vip = getElementData(checker,"vip")
	if (not vip) then return false else return true end
end
	...
	if not listdata.clickHandlersSet then
		...
		addEventHandler('onClientGUIDoubleClick', gridlist,
			function()
				...
				local dataGrid = guiGridListGetItemData(gridlist,selRow,2)
				if not selData then
					return
				end
				if tonumber(selData) == 0 then
					...
					if clickedNode[1] == 'group' then
						...
						if gridListControlData.onitemdoubleclick then
							if dataGrid == "VIP" then
								if isPlayerVIP(localPlayer) then
									gridListControlData.onitemdoubleclick(clickedNode, selRow)
								else
									outputChatBox("Ta opcja dostępna jest jedynie dla graczy VIP.",255,0,0)
									outputChatBox("Wpisz /vip i kup konto VIP bez wychodzenia z serwera.",255,0,0)
								end
							else
								gridListControlData.onitemdoubleclick(clickedNode, selRow)
						end
...
function _putLeafInGridListRow(gridlist, row, leaf, columnAttrs)
	for k,attr in ipairs(columnAttrs) do
		if tostring(leaf['vip']) == "true" then
			guiGridListSetItemText(gridlist, row, k, type(leaf[attr]) == 'boolean' and (leaf[attr] and 'X' or '') or ('    ' .. tostring(leaf[attr])), false, false)
			guiGridListSetItemData(gridlist, row, 2, "VIP")
			guiGridListSetItemColor(gridlist,row,k,255, 215, 0,255)
		else
		guiGridListSetItemText(gridlist, row, k, type(leaf[attr]) == 'boolean' and (leaf[attr] and 'X' or '') or ('    ' .. tostring(leaf[attr])), false, false)
	end
end

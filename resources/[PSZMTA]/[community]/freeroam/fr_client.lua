...
function isPlayerInAJ(cel)
    ...
    if (aj and tonumber(aj)>0) then return true else return false end
end

function isPlayerInCinema(cel)
	local d = getElementData(cel, "cinema_player")
	if (d) then return true else return false end
end
...
addCommandHandler('setstyle',
	function(cmd, style)
	if isPlayerArenaPlayed(g_Me) then return end
...
function clothesInit()
	...
	if getElementModel(g_Me) ~= 0 then
		errMsg("Musisz być w skinie CJ'a,(skin ID 0) aby założyć ubranie.")
...
function initBookmarks ()
	if isPlayerArenaPlayed(g_Me) then return end
    if isPlayerInAJ(g_Me) then return end
    if getElementDimension(g_Me) == 2430 then
        outputChatBox("Używanie teleportacji jest zabronione na mapie eventowej.",255,0,0) 
        return
    end
...
function gotoBookmark ()
	...
	if row and row ~= -1 then
        ...
        if getElementDimension(localPlayer) == 2430 then
            outputChatBox("Używanie zapisanych pozycji jest zabronione na mapie eventowej.",255,0,0) 
            return
        end
		fadeCamera(false)
		if isPedDead(g_Me) then
            ...
            local x,y,z = unpack(bookmarks[row+1])
            triggerServerEvent('checkplayers',g_Me,x,y,z,2)
		end
...
function toggleJetPack()
    if isPlayerArenaPlayed(g_Me) then return end
	if not getElementData(g_Me,"vip") then return end
    if getElementDimension(g_Me) == 2430 then
        outputChatBox("Używanie jetpack'a jest zabronione na mapie eventowej.",255,0,0) 
        return
    end
    if isPlayerInAJ(g_Me) then return end
...
function setPosClick()
	...
	--local x,y, z = getControlNumbers(wndSetPos, {'x', 'y','z'})
	--if testLineAgainstWater(x,y,z,x,y,z-0.5) then outputChatBox('halo') end
	closeWindow(wndSetPos)
end
function setPlayerPosition(x, y, z)
    ...
    if isPlayerInCinema(g_Me) then 
    	outputChatBox("Najpierw wyjdź z strefy kina.",255,0,0)
    	return
    end
	if elem then
	...
	if not hit then
		...
		if isPedDead(g_Me) then
           ...
           -- adminView_add("FR-MAP> "..getPlayerName(localPlayer)..", pos: "..pos..", gracze w obrebie 250m: ")
			if isVehicle then
...
wndSetPos = {
	...
	controls = {
		...
		{'btn', id='cancel', closeswindow=true},
		{'lbl', text='Prawym na mapę aby zamknąć.'}
	},
	oncreate = setPosInit,
...
function repairVehicle()
	local vehicle = getPedOccupiedVehicle(g_Me)
	if vehicle then
		if getVehicleOccupant(vehicle,0)~= g_Me then return end
        if getElementDimension(g_Me) == 2430 or getElementDimension(g_Me) == 121 then
            outputChatBox('Używanie naprawy pojazdu jest zablokowane na mapie eventowej.',255,0,0)
            return
        end
...
function flipVehicle()
	...
	if vehicle then
		if getVehicleOccupant(vehicle,0)~= g_Me then return end
        if getElementDimension(g_Me) == 2430 or getElementDimension(g_Me) == 121 then
            outputChatBox('Używanie obrotu pojazdu jest zablokowane na mapie eventowej.',255,0,0)
            return
        end
...
function upgradesInit()
    ...
    if getElementDimension(g_Me) == 2430 then
        outputChatBox('Tuning pojazdu jest zablokowany na mapie eventowej.',255,0,0)
        return
    end
    if getVehicleOccupant(vehicle,0)~= g_Me then return end
...
function setColorCommand(cmd, ...)
	...
	if getVehicleOccupant(vehicle,0)~= g_Me then return end
    if getElementDimension(g_Me) == 2430 then
        outputChatBox('Zmiana koloru pojazdu jest zablokowana na mapie eventowej.',255,0,0)
        return
    end
...
function openColorPicker()
	...
	if (editingVehicle) then
        ...
        if getVehicleOccupant(editingVehicle,0)~= g_Me then return end
...
function paintjobInit()
    ...
    if getElementDimension(g_Me) == 2430 then
        outputChatBox('Zmiana wyglądu pojazdu jest zablokowana na mapie eventowej.',255,0,0)
        return
    end
    if getVehicleOccupant(vehicle,0)~= g_Me then closeWindow(wndPaintjob) return end
...
function applyPaintjob(paint)
	if getVehicleOccupant(getPedOccupiedVehicle(g_Me),0)~= g_Me then closeWindow(wndPaintjob) return end

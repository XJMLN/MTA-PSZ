...
function enableWaterRef()
	...
	isMRTinPS = (vCardNumRenderTargets() > 1)
	-- Create things
    myScreenSource = dxCreateScreenSource( scx, scy )
	if isMRTinPS == true then
		imgRefractShader,tecNameX = dxCreateShader( "fx/img_refract_MRT.fx" )
		texWaterShader,tecNameY = dxCreateShader( "fx/tex_water_MRT.fx" )
		secondRT = dxCreateRenderTarget( scx, scy, true )
	else
		imgRefractShader,tecNameX = dxCreateShader( "fx/img_refract.fx" )
		texWaterShader,tecNameY = dxCreateShader( "fx/tex_water.fx" )
		secondRT = false
	end
	...
	if isMRTinPS == true then 
		bAllValid = bAllValid and secondRT
	end	
...
addEventHandler( "onClientPreRender", root,
    function()
		if not bAllValid or not secondRT then return end
...
addEventHandler( "onClientHUDRender", root, function()
	...
	if not camMatIsProper and isMRTinPS == false then 

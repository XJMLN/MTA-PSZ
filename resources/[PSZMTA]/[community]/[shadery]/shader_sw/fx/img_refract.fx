...
PSInput VertexShaderFunction(VSInput VS)
{
    ...
    float3 Tangent = VSNormal.yxz;
    Tangent.xz = VS.TexCoord.xy;
    float3 Binormal = normalize(cross(Tangent, VSNormal));
    Tangent = normalize(cross(Binormal, VSNormal));
...
float MTAApplyFade(float3 worldPos, float3 cameraPos)
{
	...
	float fogStart = min(sElementLow / 2, gFogStart);
	float fogEnd = min(sElementLow / 2, gFogEnd);
...
float4 PixelShaderFunctionSM3(PSInput PS) : COLOR0
{
    ...
    float3 specLighting = applyLiSpecular(sSunColorTop, sSunColorBott, Normal, lightDir, vView, sSpecularPower);	
    ...
    float refMul =  1 - saturate(PS.TexProj.w - depthAlt);
    ...
    // in case depthTesting fails
    float depthTest = Linearize(FetchDepthBufferValue(TexProj.xy));
    if (depthTest < PS.TexProj.w) finalColor.a = 0;
...
// Techniques
//--------------------------------------------------------------------------------------
technique dxDrawImage4D_ref_PS3_DB_noMRT
{
  pass P0
  {
    ZEnable = false;
...
technique dxDrawImage4D_ref_PS3_noDB_noMRT
{
  pass P0
  {
    ZEnable = true;
...
technique dxDrawImage4D_ref_PS2_noDB_noMRT
{
  pass P0
  {
    ZEnable = true;

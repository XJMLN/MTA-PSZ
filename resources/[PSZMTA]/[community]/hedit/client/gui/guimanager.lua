...
local function bannedVeh(veh)
	if (getElementModel(veh) == 509) or (getElementModel(veh) == 510) or (getElementModel(veh) == 481) then
		return true else return false end
end
local vDataBan = {"spawn:vehicle", "veh:fid", "owning_gang","vehicle:event","pojazd_spawn","drag:veh"}
...
function toggleEditor ( )
    ...
    if pVehicle then
        if isPublicVehicle(pVehicle) or bannedVeh(pVehicle) then  
            outputChatBox("W tym pojeździe nie możesz ustawić handlingu.",255,0,0)
            return false
        end
        if getElementData(localPlayer,"hedit_ban") then 
            outputChatBox("Aktualnie nie możesz edytować pojazdów.",255,0,0)
            return false
        end

...
local function doPoolShot(x,y,x2,y2)
	...
	if not hit or not hitElement then
		return triggerServerEvent("broadcastCaptionedEvent", localPlayer, getPlayerName(localPlayer) .. " nie trafia w żadną bilę.", 5, 5, true)
	end
	local model=getElementModel(hitElement)
	if (rodzajeBil[model]) then
		triggerServerEvent("broadcastCaptionedEvent", localPlayer, getPlayerName(localPlayer) .. " trafia w "..rodzajeBil[model].biernik..".", 5, 5, true)
	end

--[[*****************************************************************************
*
*  PROJECT:     VehicleBlips
*  LICENSE:     GNU GPLv3
*  FILE:        :VehicleBlips/cVehicleBlips.lua
*  PURPOSE:     Vehicle blips.
...
local function resourceStart()
	for _, vehicle in ipairs(getElementsByType("vehicle"), root, true) do
		if vehicle ~= getPedOccupiedVehicle(localPlayer) then
			local blip = createBlipAttachedTo(vehicle, 0, 1, 150, 150, 150, 255, -10, 75)
...
local function streamIn()
	...
	local blip = createBlipAttachedTo(source, 0, 1, 150, 150, 150, 255, -10, 75)
	setElementParent(blip, vehicleBlipRoot)

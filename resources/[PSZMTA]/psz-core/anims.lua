...
function walkingAnims(plr,cmd,style)
    style = style and tonumber(style)
    if style then
   -- for i,v in ipairs(walkingID) do
            if not walkingID[style] then outputChatBox("Wprowadziłeś zły numer, spis dostępnych numerów styli dostępny jest pod F9.",plr,255,0,0) return false end
        if style == 0 then outputChatBox("Ustawiłeś standardowy styl chodzenia.",plr,188,188,188) setPedWalkingStyle(plr,style) return false end
        outputChatBox("Zmieniono styl chodzenia na numer: "..style..".",plr,188,188,188)
        setPedWalkingStyle(plr,walkingID[style])
       -- end
    end
end
addCommandHandler("styl",walkingAnims)
